...
(* Student name: Derek Ing *)
(* CMS cluster login name: ding *)
...
let is_solved b =
  let check = fun c p -> LocSet.equal p (LocSet.of_list [(3,1); (4,1); (3,2); (4,2)])
  in CharMap.exists check b.pieces

let compare b1 b2 =
  let comp = LocSet.compare b1.unoccupied b2.unoccupied in
    if comp <> 0 then comp
    else
      let rec helper = function
        | [] -> LocSetSet.empty
        | (_, locs) :: t ->  LocSetSet.add locs (helper t)
    in 
      let pieces1 = helper (CharMap.bindings b1.pieces) in
      let pieces2 = helper (CharMap.bindings b2.pieces) in 
  ...
  if CharMap.mem c b.pieces && i > 0
    ...
    in let move n d (r, c) =
      match d with
        ...
        | Right -> (r, c + n)
    in 
      let rec check_move n d (r, c) =
        let (r', c') = move n d (r, c) in
          if n = 0 then true
          else (LocSet.mem (r', c') (LocSet.union pieces b.unoccupied)) 
            && r' >= 0 && r' <= 4 && c' >= 0 && c' <= 4 && check_move (n - 1) d (r, c)
    in let locs = LocSet.map (move i d) pieces
  in if LocSet.for_all (check_move i d) pieces 
...
let next b =
  let rec moves c i d = 
    match make_move (c, d, i) b with
      | None -> []
      | Some b' -> b' :: moves c (i + 1) d
  in let map_func (c, locs) = List.concat_map (moves c 1) [Up; Down; Left; Right]
