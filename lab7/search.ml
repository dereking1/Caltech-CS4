...
module Search (S : Storage) (D : Domain) =
  struct
    ...
    let search init =
      ...
      let new_history = [init] in S.push new_history storage;
        ...
        let rec iter prev_boards =
          if S.is_empty storage
            then raise Not_found
              ...
              if DS.mem last_board prev_boards then iter prev_boards
              else if D.is_solved last_board then next_history
              else
                begin
